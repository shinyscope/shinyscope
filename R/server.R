...
# this allows lubridate values to be saved in the dataframe
  ...
  dynamic_ui_categories <- function(categories_table) {
    #this makes edit_button_n as identifier for actionbutton n being the nth row
    button_ids <- reactive({
    ...
    lapply(1:nrow(categories_table), function(i) {
      ...
      # unique id for each edit_button i.e. edit_button_1
      ...
      observeEvent(input[[edit_button_id]], {
        button_states[[edit_button_id]] <- isolate({
          ...
          #used num so I can access all previous setup for edit modal.
          num <- as.numeric(strsplit(edit_button_id, "_")[[1]][3]) #regex - it takes the number value from the ID at the end
